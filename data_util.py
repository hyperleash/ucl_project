...
class MimicFullDataset(Dataset):
    def __init__(self, version, mode, vocab_path, truncate_length, 
                 label_truncate_length=30, term_count=1, sort_method='max', summarised = False, summary_name = 'summary_extractive_addition'):
        ...
        if version in ['mimic3', 'mimic3-50']:
            filename = f"{version}_{mode}"
            if summarised:
                filename += "_summarised"
            
            self.path = os.path.join(MIMIC_3_DIR, f"{filename}.json")
    ...
    def getsummary(self, index):
        if(self.summarised):
            return self.df[index][self.summary_name]
        else:
            return ""
        
    
    def getaddition(self, index):
        #CURRENTLY IS NOT USED
        return self.df[index]['Addition']
    ...
    def __get_text_label__(self, index):
        ...
        summary = self.getsummary(index)
        addition = self.getaddition(index)
        label = str(self.df[index]['LABELS']).split(';')
        return text, addition, summary, label
    ...
    def __getitem__(self, index):
        text, addition, summary, label = self.__get_text_label__(index)

        
        processed_summary = self.process(summary, label)
        processed_text = self.process(text, label)
...
def aweful_collate_function(batch):
    return my_collate_fn([batch[0][0]]), my_collate_fn([batch[0][1]])
