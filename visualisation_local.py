...
def format_classname(classname):
    return '<td style="vertical-align: top;"><text style="padding-right:2em"><b>{}</b></text></td>'.format(classname)
    ...
    for datarecord in datarecords:
        rows.append(
            "".join(
                [
                    "<tr>",
                    f'{format_classname(datarecord.true_class)}',
                    f'{format_classname("{0} ({1:.2f})".format(datarecord.pred_class, datarecord.pred_prob))}',
                    f'{format_classname("{0:.2f}".format(datarecord.attr_score))}',
                    f'{format_word_importances(datarecord.raw_input_ids, datarecord.word_attributions)}',
                    "<tr>",
                ]
            )
        )
